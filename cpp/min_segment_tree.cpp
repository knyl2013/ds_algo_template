...
public:
    ...
    SegTreeNode* build_tree(int start, int end) {
        ...
        return new SegTreeNode(start, end, min(left->val, right->val), left, right);
    }
    ...
    int range_query(SegTreeNode* root, int start, int end) {
        ...
        if (on_left) {
            ...
            return min(range_query(root->left, start, root->left->end), range_query(root->right, root->right->start, end));
        }
    }
...
int main() {
    ...
    MinSegTree* segTree = new MinSegTree(inputs);
    cout << segTree->range_query(1, 4) << endl;
}